...
function buildDebugHeader({ skillName, skillTitle, generatedOn, ruleCount }) {
  return [
    "<!--",
    "DO NOT EDIT THIS FILE DIRECTLY.",
    "",
    "This debug file is generated from ./.shared-rules/*.md and ./rules/*.md",
    "Edit the rule source files instead, then recompile.",
    "",
    `Generated on: ${generatedOn}`,
    "-->",
    "",
    `# ${skillTitle} — Agent Rule Debug`,
    "",
    "## Overview",
    `Debug artifact for \`${skillName}\` showing all resolved rule blocks (both \`inline\` and \`reference\`) in evaluation order.`,
    "",
    `Total rules in this document: ${ruleCount}.`,
    "",
    "## Rule Lookup Index",
    "<!-- GENERATED_RULE_INDEX -->",
    "",
    "## All Rule Blocks (Debug)",
    "<!-- GENERATED_DEBUG_RULES -->",
  ].join("\n");
}
...
function buildDebugRules(entries) {
  if (!entries.length) return "- (none)";
  return entries
    .map((entry) => {
      const heading = `#### Source: \`${entry.sourcePath}\` | Mode: \`${entry.indexMode}\``;
      return [heading, "", reheadingRuleBlock(entry.blockText)].join("\n");
    })
    .join("\n\n---\n\n");
}
...
function buildAgentsDebugOutput({ skillName, skillTitle, ruleEntries }) {
  const header = buildDebugHeader({
    skillName,
    skillTitle,
    generatedOn: formatDate(),
  ...
  return header
    .replace("<!-- GENERATED_RULE_INDEX -->", buildRuleIndex(ruleEntries))
    .replace("<!-- GENERATED_DEBUG_RULES -->", buildDebugRules(ruleEntries));
}
...
async function buildSkillArtifacts(skillDir, sharedSources) {
  ...
  return {
    ...
    agentsDebugContent: buildAgentsDebugOutput({
      skillName,
      skillTitle,
      ruleEntries,
    }),
...
async function run() {
  ...
  for (const skillDir of skillDirs) {
    ...
    const agentsDebugPath = path.join(skillsRoot, skillDir, "AGENTS.debug.md");
    const expectedDebug = `${artifacts.agentsDebugContent}\n`;
    await writeFile(agentsDebugPath, expectedDebug);
  }
