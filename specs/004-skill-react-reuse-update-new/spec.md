...
- Import/dependency direction, canonical endpoint-layer semantics, alias-path boundary enforcement, and documented generic fetch-hook exception handling are inherited from `shared/rules/30_architecture_boundaries.md`.
...
- Access and write-control behavior (including architecture/spec-document edit restrictions) is inherited from `shared/rules/130_access_write_control.md`.
...
- Shared policy rule modules `shared/rules/30_architecture_boundaries.md` and `shared/rules/130_access_write_control.md` remain available and authoritative for inherited boundary/write constraints.
