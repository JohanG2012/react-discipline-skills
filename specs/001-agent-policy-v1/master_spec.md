# Master Spec: 001 Agent Policy Baseline

This document aggregates sections that map to spec `001-agent-policy-v1`.
...
- [Before Coding Workflow](#before-coding-workflow)
...
- The normative policy baseline begins at `## Non-negotiables` and continues
...
- If appendix text ever conflicts with normative policy text, the normative
  policy sections take precedence.
...
- This policy baseline does not implement feature code.
- This policy baseline does not mandate tooling changes unless explicitly
  requested as dedicated work.

### Terminology

- **Page**: route entry point and thin orchestrator.
- **Section**: feature-owned, domain-aware UI orchestration.
- **Primitive**: reusable UI building block with no domain behavior.
- **Composite**: reusable UI pattern that stays domain-agnostic.
- **DTO**: raw wire-format transport type for API boundaries.
- **Domain model**: feature-owned semantic representation and logic.
