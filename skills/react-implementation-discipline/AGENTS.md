...
| `sr-effects-run-once-hacks` | No Run-Once Effect Hacks | MUST | `reference` | No Run-Once Effect Hacks. | Preserves strict dependency modeling and avoids fragile run-once behavior under StrictMode and refactor churn. | `.shared-rules/490_async_effects_and_component_reliability.md` |
| `sr-async-gating-not-state-only` | Async Gating Must Not Rely on State Alone | MUST | `reference` | Async Gating Must Not Rely on State Alone. | React state updates are not a reliable mutex for fast, repeated trigger sources. | `.shared-rules/490_async_effects_and_component_reliability.md` |
| `sr-async-race-protection` | Async Race and Out-of-Order Response Protection | MUST | `reference` | Async Race and Out-of-Order Response Protection. | Prevents stale async responses from corrupting current UI/data state. | `.shared-rules/490_async_effects_and_component_reliability.md` |
| `sr-async-event-handler-discipline` | Async Event Handler Discipline | MUST | `reference` | Async Event Handler Discipline. | Keeps JSX declarative and async error handling consistent across boundaries. | `.shared-rules/490_async_effects_and_component_reliability.md` |
| `sr-observer-listener-stability` | Observer and Listener Stability | MUST | `reference` | Observer and Listener Stability. | Prevents performance churn and duplicate side-effects from frequent teardown/recreate cycles. | `.shared-rules/490_async_effects_and_component_reliability.md` |
| `sr-react-list-key-stability` | React List Key Stability | MUST | `reference` | React List Key Stability. | Stable keys preserve identity semantics during list updates and avoid subtle UI/state bugs. | `.shared-rules/490_async_effects_and_component_reliability.md` |
| `sr-pagination-cursor-opaque` | Pagination Cursor Must Be Opaque | MUST | `reference` | Pagination Cursor Must Be Opaque. | Opaque cursor semantics prevent duplicate/skip bugs caused by field-coupled client pagination. | `.shared-rules/490_async_effects_and_component_reliability.md` |
| `sr-component-responsibility-split` | Component Responsibility Split | MUST | `reference` | Component Responsibility Split. | Prevents orchestration-heavy god components and keeps ownership split between hooks and presentation. | `.shared-rules/490_async_effects_and_component_reliability.md` |
| `sr-error-gating-isolation` | Error Gating Isolation | MUST | `reference` | Error Gating Isolation. | Keeps failures scoped so unrelated feature flows remain usable and recoverable. | `.shared-rules/490_async_effects_and_component_reliability.md` |
